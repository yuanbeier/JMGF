...
#pragma warning restore 618
        ...
        static readonly MethodInfo
                    ...
                    //Add Code 20160803
                    guidParse = typeof(Guid).GetMethod("Parse", new Type[] { typeof(string) }),
            ...
            foreach (var item in members)
            {
                if (item != null)
                {
                    ...
                    else
                    {
                        ...
                        //新增字符串转Guid(Guid?)类型
                        else if (unboxType == typeof(Guid) && colType == typeof(string))
                        {
                            if (guidDeclareLocal == -1)
                            {
                                guidDeclareLocal = il.DeclareLocal(typeof(string)).LocalIndex;
                            }
                            il.Emit(OpCodes.Castclass, typeof(string)); // stack is now [target][target][string]
                            StoreLocal(il, guidDeclareLocal); // stack is now [target][target]                            
                            LoadLocal(il, guidDeclareLocal); // stack is now [target][target][enum-type][string]                            
                            il.EmitCall(OpCodes.Call, guidParse, null); // stack is now [target][target][enum-as-object]                           
                            if (nullUnderlyingType != null)
                            {
                                il.Emit(OpCodes.Newobj, memberType.GetConstructor(new[] { nullUnderlyingType })); // stack is now [target][target][typed-value]
                            }
...
#pragma warning restore 612, 618
    ...
    sealed partial class DefaultTypeMap : SqlMapper.ITypeMap
    {
        ...
        public SqlMapper.IMemberMap GetMember(string columnName)
        {
            ...
            if (property == null)
            {
                if (columnName.StartsWith("c_",StringComparison.CurrentCultureIgnoreCase))
                {
