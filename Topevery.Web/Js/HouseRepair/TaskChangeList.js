...
TaskChangeList = {
    ...
    ///删除
    Delete: function () {
        var arrRows = $('#TaskChangeListTable').datagrid('getSelections');
        if (arrRows.length === 0) {
            $.messager.alert('提示', '请选择一条需要删除的记录!', 'info');
        } else {
            var ids = [];
             var url = virtualDirName + 'api/services/app/RepairTaskW/DeleteRepairAsync';
            $.each(arrRows, function () {
                ids.push(this.id);
            });
            deleteAjax(ids.join(), "TaskChangeListTable", url);
        }
    },
    ///初始化列表
    Initialize: function () {
        grid = $('#TaskChangeListTable').datagrid({
            height: 480,
            idField: "id",
            striped: true,
            fitColumns: true,
            ...
            nowrap: false,
            fit:true,
            loader: TaskChangeList.loadData,
            rownumbers: false, //行号
            ...
            columns: [
                [
                    ...
                    { field: "unit", title: "单位", align: "center", width: "5%", sortable: true },
                    { field: "num", title: "数量", align: "center", width: "4%", sortable: true },
                    { field: "unitPriceY", title: "单价", align: "center", width: "6%", sortable: true },
    ...
    ///查询功能
    loadInfo: function () {
        $('#TaskChangeListTable').datagrid('load', { input: topevery.form2Json("selectForm") }); //点击搜索
    }
}
///初始化加载方法
$(function () {
    TaskChangeList.Initialize();
