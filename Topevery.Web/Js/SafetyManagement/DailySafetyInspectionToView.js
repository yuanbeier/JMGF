DailySafetyInspectionTiew= {
    Initialize:function() {
        topevery.ajax({
            type: "POST",
            url: "api/services/app/SafetyR/GetSafetyTypeInfoAsync",
            ...
            if (data.success) {
                ...
                for (var i = 0; i < row.length; i++) {
                    ...
                    topevery.PicturesShow(row[i].id, 23, "Attachment_" + row[i].checkProjectId + "");
                }
                topevery.PicturesShow(data.result.id, 22, "Attachment");
                var array = {};
