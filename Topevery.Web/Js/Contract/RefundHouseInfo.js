    ...
    LeaseReviewSave: function () {
        if (isupload) {
            if ($("input[name='IdhiddenFile']").val() === "") {
                window.top.topeveryMessage.show("请上传解除房屋租赁合同审批表，退还房屋现场勘验确认表！");
                return;
            }
        }
        topevery.ajax({
            type: "POST",
            url: "api/services/app/RefundHouseApplyW/AddWfInstanceLink",
            contentType: "application/json",
    ...
    loadData: function() {
        topevery.ajax({
            ...
            if (row.success) {
                ...
                $("#removeReason").html(data.removeReason).attr("title", data.removeReason);
...
function Initializecc(id) {
    topevery.ajax({
    type: "POST",
    url: "api/services/app/RefundRentApplyR/GetRefundRentApplyHandleInfo",
    contentType: "application/json",
    data: JSON.stringify({
        "ActivityInstanceId": id ,
        "RefundRentApplyId": "",
        "FlowInstanceId": "",
        "HouseId": topevery.getQuery("houseId")
    })
}, function (data) {
    if (data.success) {
